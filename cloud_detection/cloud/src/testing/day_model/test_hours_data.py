...
def test_day_hours(base_folder, model_file, input_folders, label_folder, date):
    """
    Test Day Model on Hourly Data for exact hour timestamps.
    """
    ...
    for hour in range(24):  # 24시간
        time_suffix = f"{hour:02}00"
        time_stamp = f"{date}{time_suffix}"  # 정각 시간대만 사용
    ...
    if accuracies:
        ...
        print(f"\nAverage Accuracy for hourly data on {date}: {average_accuracy * 100:.2f}%")
